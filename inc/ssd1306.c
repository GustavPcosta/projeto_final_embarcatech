...
void move_square(ssd1306_t *ssd, uint8_t size, uint8_t speed) {
  uint8_t x = 0, y = 0;
  int8_t dx = speed, dy = speed; 

  while (1) {
      ssd1306_fill(ssd, 0); // Limpa a tela
      ssd1306_rect(ssd, y, x, size, size, 1, 1); // Desenha o quadrado
      ...
      if (x + size >= ssd->width || x == 0) dx = -dx;
      if (y + size >= ssd->height || y == 0) dy = -dy;

      sleep_ms(50); // Ajuste para controlar a velocidade do movimento
  }
}